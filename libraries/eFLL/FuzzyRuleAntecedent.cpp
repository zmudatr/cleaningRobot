...
//
	this->fuzzyRuleAntecedent3 = NULL;
//
	}
...
bool FuzzyRuleAntecedent::joinWithAND(FuzzySet *fuzzySet1, FuzzySet *fuzzySet2, FuzzySet *fuzzySet3)
{
    // check if three FuzzySet are valid
    if (fuzzySet1 != NULL  && fuzzySet2 != NULL && fuzzySet3 != NULL)
    {
        // set the mode and references
        this->op = OP_AND3;
        this->mode = MODE_FS_FS;
...
float FuzzyRuleAntecedent::evaluate()
{
    ...
    case MODE_FS_FS:
        ...
        case OP_AND:
			...
			case OP_AND3:
            // case the operator is AND, check if both has pertinence bigger then 0.0
            if (this->fuzzySet1->getPertinence() > 0.0 && this->fuzzySet2->getPertinence() > 0.0 && this->fuzzySet3->getPertinence() > 0.0)
            {
                // in this case, return the small pertinence between two FuzzySet
                if (this->fuzzySet1->getPertinence() <= this->fuzzySet2->getPertinence())
                {
                  	if (this->fuzzySet1->getPertinence() <= this->fuzzySet3->getPertinence())
					{
						return this->fuzzySet1->getPertinence();
					}
					else
					{
						return this->fuzzySet3->getPertinence();
					}	
                ...
                {
					if (this->fuzzySet2->getPertinence() <= this->fuzzySet3->getPertinence())
					{					
						return this->fuzzySet2->getPertinence();
					}
					else
